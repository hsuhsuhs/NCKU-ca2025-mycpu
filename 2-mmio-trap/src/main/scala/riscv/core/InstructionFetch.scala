...
class InstructionFetch extends Module {
  ...
  val next_pc_sequential = pc + 4.U(Parameters.AddrWidth)
  ...
  when(io.instruction_valid) {
    ...
    pc := Mux(
      io.interrupt_assert,
      io.interrupt_handler_address, // 1. Highest Priority: Interrupt
      Mux(
        io.jump_flag_id,
        io.jump_address_id,         // 2. Second Priority: Control Flow (Jump/Branch Taken)
        next_pc_sequential          // 3. Lowest Priority: Sequential Execution (PC + 4)
      )
    )
