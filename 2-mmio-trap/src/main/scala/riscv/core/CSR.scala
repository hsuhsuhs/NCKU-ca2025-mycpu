 ...
 * When both CLINT and CPU attempt to write CSRs simultaneously,
...
class CSR extends Module {
  ...
  val regLUT =
    IndexedSeq(
      // Mapped CSR addresses to register wires
  ...
  when(io.clint_access_bundle.direct_write_enable) {
    ...
    // TODO: Which CSRs does CLINT need to write? (FIX: Replace '?' with actual Reg names)
