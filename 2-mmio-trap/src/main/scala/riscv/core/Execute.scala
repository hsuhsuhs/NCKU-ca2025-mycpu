...
class Execute extends Module {
  ...
  // ============================================================
  // FIX: Define signed and unsigned versions of register data for Branch comparisons
  // This resolves the "not found" errors for reg1_u, reg2_u, reg1_s, reg2_s
  // ============================================================
  // Unsigned values (BEQ, BNE, BLTU, BGEU)
  val reg1_u = io.reg1_data
  val reg2_u = io.reg2_data
  // Signed values (BLT, BGE)
  val reg1_s = io.reg1_data.asSInt
  val reg2_s = io.reg2_data.asSInt
  ...
  val branchCondition = MuxLookup(funct3, false.B)(
    Seq(
      // TODO: Implement six branch conditions (Completed using defined variables reg1_u/s, reg2_u/s)
      // Hint: Compare two register data values based on branch type
      // Equality/Inequality comparison (sign-agnostic)
      InstructionsTypeB.beq  -> (reg1_u === reg2_u),
      InstructionsTypeB.bne  -> (reg1_u =/= reg2_u),

      // Signed comparison
      InstructionsTypeB.blt  -> (reg1_s < reg2_s),
      InstructionsTypeB.bge  -> (reg1_s >= reg2_s),

      // Unsigned comparison
      InstructionsTypeB.bltu -> (reg1_u < reg2_u),
      InstructionsTypeB.bgeu -> (reg1_u >= reg2_u)
    )
  )
  ...
  val branchTarget = (io.instruction_address.asSInt + io.immediate.asSInt).asUInt
  ...
  val jalrSum      = (io.reg1_data.asSInt + io.immediate.asSInt).asUInt

  // TODO: Clear LSB using bit concatenation
  // Hint: Extract upper bits and append zero
  // FIX: Clear the LSB (bit 0) by taking bits [DataBits-1: 1] and concatenating with 0
