...
class WriteBack extends Module {
  ...
  io.regs_write_data := MuxLookup(io.regs_write_source, io.alu_result)(
    Seq(
      // RegWriteSource.Memory (1): Use data from Load instruction
      RegWriteSource.Memory               -> io.memory_read_data,
      // RegWriteSource.CSR (2): Use data read from CSR register
      RegWriteSource.CSR                  -> io.csr_read_data,
      // RegWriteSource.NextInstructionAddress (3): Use PC + 4 (return address for JAL/JALR)
      RegWriteSource.NextInstructionAddress -> (io.instruction_address + 4.U(Parameters.DataWidth))
    )
  )
}
