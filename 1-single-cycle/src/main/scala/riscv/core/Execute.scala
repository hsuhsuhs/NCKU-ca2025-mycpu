...
class Execute extends Module {
  ...
  val branchCondition = MuxLookup(funct3, false.B)(
    Seq(
      ...
      // BEQ: Equality (rs1 == rs2)
      InstructionsTypeB.beq  -> (io.reg1_data === io.reg2_data),
      // BNE: Inequality (rs1 != rs2)
      InstructionsTypeB.bne  -> (io.reg1_data =/= io.reg2_data),

      // Signed comparison (requires conversion to SInt)
      // BLT: Less Than (rs1 < rs2, signed)
      InstructionsTypeB.blt  -> (io.reg1_data.asSInt < io.reg2_data.asSInt),
      // BGE: Greater than or Equal (rs1 >= rs2, signed)
      InstructionsTypeB.bge  -> (io.reg1_data.asSInt >= io.reg2_data.asSInt),

      // Unsigned comparison (UInt comparison is default)
      // BLTU: Less Than Unsigned (rs1 < rs2, unsigned)
      InstructionsTypeB.bltu -> (io.reg1_data < io.reg2_data),
      // BGEU: Greater than or Equal Unsigned (rs1 >= rs2, unsigned)
