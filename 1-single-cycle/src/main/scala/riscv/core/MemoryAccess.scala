...
class MemoryAccess extends Module {
  ...
  // Bits remaining after loading a Byte or Halfword for extension
  val remainingBitsByte = Parameters.DataBits - ByteBits  
  val remainingBitsHalf = Parameters.DataBits - HalfwordBits
  ...
  when(io.memory_read_enable) {
    ...
    val byteSignBit = byte(ByteBits - 1)
    val halfSignBit = half(HalfwordBits - 1)
  ...
  // - SH to address 0x1002 (index=2): data[15:0] → bytes 2-3, strobes[2:3]=1
    ...
    // SB (Store Byte) shift amount: mem_address_index * 8 bits
    val byteShiftAmount = Cat(mem_address_index, 0.U(3.W))
    ...
    switch(io.funct3) {
      is(InstructionsTypeS.sb) {
        ...
        writeData := data(Parameters.ByteBits - 1, 0) << byteShiftAmount
      }
      is(InstructionsTypeS.sh) {
        ...
        when(mem_address_index(1) === 0.U) {
          ...
          writeData := data(HalfwordBits - 1, 0) // No shift needed
        }.otherwise {
          ...
          writeData := data(HalfwordBits - 1, 0) << HalfwordBits.U // Shift by 16
        }
      }
