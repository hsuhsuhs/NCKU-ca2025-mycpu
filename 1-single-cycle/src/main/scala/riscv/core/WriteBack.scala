...
class WriteBack extends Module {
  ...
  val next_instruction_address = io.instruction_address + 4.U
  ...
  io.regs_write_data := MuxLookup(io.regs_write_source, io.alu_result)(
    Seq(
      // RegWriteSource.Memory (1): Load instruction result
      RegWriteSource.Memory             -> io.memory_read_data,

      // RegWriteSource.NextInstructionAddress (2): JAL/JALR return address (PC+4)
      RegWriteSource.NextInstructionAddress -> next_instruction_address
    )
  )
}
